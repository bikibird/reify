...
// #region mood
reify.lang.indicative=0 //I ATE
reify.lang.imperative=1 //You, EAT!
reify.lang.subjunctive=2 //I WOULD HAVE EATEN

reify.tense=reify.lang.mood=reify.lang.indicative //DEFECT REMOVE
// #endregion

// #region voice
reify.lang.active=0 //I ATE
...
reify.lang.a=function(word)
{
	var _trie=reify.lang.a.trie
...
reify.lang.es=function(verbStem)
{
	...
	var _trie=reify.lang.es.trie
	var pattern=("$"+verb).split("").reverse().join("")
	for (let letter of pattern) 
	{
		if(_trie.hasOwnProperty(letter))
		{
			if( typeof _trie[letter]==="number")
			{
				return (rules[_trie[letter]](verb)+" "+particles).trim()
		...
		{
			if (_trie.hasOwnProperty("$")){return (rules[_trie.$](verb)+" "+particles).trim()}
			else {return (rules[0](verb)+" "+particles).trim()}
		}
	}
}
reify.lang.es.trie={h:{p:{a:{r:{g:{o:{e:{m:1}},$:0}}}},c:{n:{u:1,y:{l:1},i:1,e:1,a:1},a:{m:0,$:1},y:0,$:1},s:1},s:{a:{g:{t:9,e:9,$:1},i:1},i:{c:1,l:1,w:1,o:1,d:9},u:{l:{l:1,p:9},c:1,o:1,b:1,r:1},o:9,m:0,$:1},y:{a:0,o:0,e:0,u:0,$:2},x:1,i:{p:1,x:1},o:{d:{a:{n:{i:1}},$:1},g:1,t:{e:1},h:{c:1},i:{d:1},a:1,y:{o:1},s:1,e:{d:1},r:1},f:{l:{a:5},a:{e:{h:5}}},z:{i:3,z:1,t:1},u:{f:1}}

reify.lang.es.third={have:"has","be":"is"}
reify.lang.ed=function(verbStem)
{
	let verb=verbStem.toLowerCase().split(" ")
	let particles=verb.splice(1).join(" ")
	verb=verb[0]
	if(reify.lang.ed.verbs.hasOwnProperty(verb)){return reify.lang.preserveCase((reify.lang.ed.verbs[verb]+" "+particles).trim(),verbStem)}
...
reify.lang.s=function(word)
{
	var words = word.split(" ")
	var plural =words[0]
	var remainder =words.slice(1).join(" ")
	var key=plural.toLowerCase()
	if(reify.lang.s.nouns.hasOwnProperty(key)){return reify.lang.preserveCase(reify.lang.s.nouns[key],plural)+ (remainder?" "+remainder:"") }
...
reify.lang.er=function(word)
{
	var key=word.toLowerCase()
	if(reify.lang.er.comparatives.hasOwnProperty(key)){return reify.lang.preserveCase(reify.lang.er.comparatives[key],word) }
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tvwxz]$/)){return reify.lang.preserveCase(key+key.charAt(key.length-1)+"er",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][aeiou]+[b-df-hj-np-tvwxz]$/)){return reify.lang.preserveCase(key+"er",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tv-z]e$/)){return reify.lang.preserveCase(key+"r",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tvwxz]*y$/)){return reify.lang.preserveCase(key.slice(0,-1)+"ier",word)}
	return reify.lang.preserveCase("more "+key,word)
}
reify.lang.er.comparatives={bad:"worse",far:"farther",good:"better"}

reify.lang.est=function(word)
{
	var key=word.toLowerCase()
	if(reify.lang.est.superlatives.hasOwnProperty(key)){return reify.lang.preserveCase(reify.lang.est.superlatives[key],word) }
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tv-z]$/)){return reify.lang.preserveCase(key+key.charAt(key.length-1)+"est",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][aeiou]+[b-df-hj-np-tv-yz]$/)){return reify.lang.preserveCase(key+"est",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tv-z]e$/)){return reify.lang.preserveCase(key+"st",word)}
	if(key.match(/(un)?[b-df-hj-np-tv-z]+[aeiou][b-df-hj-np-tv-z]*y$/)){return reify.lang.preserveCase(key.slice(0,-1)+"iest",word)}
	return reify.lang.preserveCase("most "+key,word)
}
reify.lang.est.superlatives={bad:"worst",far:"farthest",good:"best"}
// #endregion
